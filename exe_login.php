...
//echo ($username . " - ". md5($password));
	...
	$chk_username = "SELECT * FROM `std_info` where `username` = '".$username."' and `password` = '".md5($password)."' and `approve` = '1'";
...
//Check whether the query was successful or not
		...
		if($num_rows_username == 1) {
			...
			$_SESSION['SESS_STD_ID'] = $member['std_id'];
			$_SESSION['SESS_STD_NAME'] = $member['std_name'];
