...
require_once 'include/env_helper.php';
?>
<!doctype html>
<html>
<head>
...
$(function() {      
  ...
  $("#result").hide();
	...
	$("#frm_std_info").on('submit',(function(e) {	
		e.preventDefault();
		//alert($("#frm_std_info select[id=district_id]").val());
	
				$.ajax ({
					type: "POST", 
					url: "exe_std_info.php", 
					cache: false, 
					async: false, 							
					data:  {
									std_prefix: $("#frm_std_info input[name=std_prefix]:checked").val(),
									std_name:$("#frm_std_info input[id=std_name]").val(),
									std_sname:$("#frm_std_info input[id=std_sname]").val(),
									std_group:$("#frm_std_info input[id=std_group]").val(),
									std_mobile:$("#frm_std_info input[id=std_mobile]").val(),	
									std_email:$("#frm_std_info input[id=std_email]").val(),	
									office_id:$("#frm_std_info select[id=office_id]").val(),
									lat_office:$("#frm_std_info input[id=lat_office]").val(),
									lng_office:$("#frm_std_info input[id=lng_office]").val()									
								},
	...
	$.ajax ({
      ...
      function initMap() {
		...
		if ((lati == "") && (long == ""))  {
			if (navigator.geolocation) {
			  navigator.geolocation.getCurrentPosition(function(position) {
				...
				$("#frm_std_info input[id=lat_office]").val(pos.lat);
				$("#frm_std_info input[id=lng_office]").val(pos.lng);
		...
		//	alert("Latitude: " + event.latLng.lat() + " " + ", longitude: " + event.latLng.lng());
		  $("#frm_std_info input[id=lat_office]").val(event.latLng.lat());
		  $("#frm_std_info input[id=lng_office]").val(event.latLng.lng());			
        });
...
<div class="container">
  ...
  <div class="row" id="register">
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
      <form id="frm_std_info" action="" method="POST">
          ...
          <div class="col-md-4">
            <input type="text" id="std_id" name="std_id" class="form-control" maxlength="9" readonly required placeholder="รหัสประจำตัวนักศึกษา" />
          </div>
...
<script async defer src="https://maps.googleapis.com/maps/api/js?key=<?php echo getEnvVar('GOOGLE_MAPS_API_KEY'); ?>&callback=initMap"
    ></script>
			<div class="col-md-4"> </div>
        </div> 
...
<?php
  ...
  $sql = "SELECT * FROM std_info WHERE std_id = '".$std_id."'";
...
?>
							<script type="text/javascript">
									...
									$("#frm_std_info input[id=std_id]").val('<?php echo $row['std_id']; ?>');
								
									if("<?php echo $row['std_prefix']; ?>" == "นาย"){$("#frm_std_info input[id=std_prefix1]").prop('checked', true);}
									if("<?php echo $row['std_prefix']; ?>" == "นาง"){$("#frm_std_info input[id=std_prefix2]").prop('checked', true);}
									if("<?php echo $row['std_prefix']; ?>" == "นางสาว"){$("#frm_std_info input[id=std_prefix3]").prop('checked', true);}
									$("#frm_std_info input[id=std_name]").val('<?php echo $row['std_name']; ?>');
									$("#frm_std_info input[id=std_sname]").val('<?php echo $row['std_sname']; ?>');					
									$("#frm_std_info input[id=std_group]").val('<?php echo $row['std_group']; ?>');	
									$("#frm_std_info input[id=std_mobile]").val('<?php echo $row['std_mobile']; ?>');
									$("#frm_std_info input[id=std_email]").val('<?php echo $row['std_email']; ?>');		
								
									$("#frm_std_info select[id=office_id]").val('<?php echo $row['office_id']; ?>');
								
								    $("#frm_std_info input[id=lat_office]").val('<?php echo $row['lat_office']; ?>');
									$("#frm_std_info input[id=lng_office]").val('<?php echo $row['lng_office']; ?>');
