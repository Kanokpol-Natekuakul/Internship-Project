...
//echo ($username . " - ". md5($password));
	...
	$chk_username = "SELECT * FROM `user` where `username` = '".$username."' and `password` = '".md5($password)."' and `permission` = '1'";
...
//Check whether the query was successful or not
		...
		if($num_rows_username == 1) {
			...
			$_SESSION['SESS_AUDITOR_ID'] = $member['id'];
			$_SESSION['SESS_AUDITOR_NAME'] = $member['name'];
			$_SESSION['SESS_AUDITOR_USERNAME'] = $member['username'];
