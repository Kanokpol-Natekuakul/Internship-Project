...
$(function() {  
  ...
  $("#btn_add_info").on("click", function(){
		...
		if ($("#frm_add_info textarea[name=work]").val()=='') {
			$("#warndata").find('.modal-body').text('กรุณากรอกรายละเอียดการทำงาน');
			$("#warndata").modal('show');
		} else {
			$("#adddata").modal('hide');
			$.ajax({
				url: "exe_work_info.php",
				type: "POST",
				data:  {	
								datework:$("#frm_add_info input[id=dateadd]").val(),
								work:$("#frm_add_info textarea[name=work]").val()
								},
				cache: false,
  ...
  $("#btn_del_info").on("click", function(){
		...
		//alert(evidenceid);
    	$("#adddata").modal('hide');
		$("#deldata").find('.modal-body').text('ยืนยันการลบข้อมูล วันที่ : ' + editdatework);
		$("#deldata").modal('show');
		//var chk_listl3_id = $("#chk_listl3_id").val();
		//alert(datework);
	    //alert($("#frm_add_info textarea[name=work]").val());
  ...
  $("#btn_condel_info").on("click", function(){
    	//callback(false);
		//alert(evidenceid);
    	$("#deldata").modal('hide');
		//var chk_listl3_id = $("#chk_listl3_id").val();
					 ...
					 success: function(data)
							{
								if (data == 0) {
									$("#frm_add_info")[0].reset();	
									$("#frm_add_info textarea[name=work]").prop('readonly', false);
	...
	/*dateClick: function(info) {
    var datework = info.dateStr;
    var today = new Date();  // Get the current date
    var selectedDate = new Date(info.dateStr);  // Convert the selected date
    today.setHours(0, 0, 0, 0); 
    // Check if selected date is in the past
    if (selectedDate < today) {
        $("#warndata").find('.modal-body').text('ไม่สามารถบันทึกข้อมูลย้อนหลังได้');
        $("#warndata").modal('show');
        return;  // Prevent further execution if the selected date is in the past
    }

    $.ajax({
        url: "chk_work_info.php",
        type: "POST",
        data: { datework: datework },
        cache: false,
        success: function(data) {
            if (data == 0) {
                $("#frm_add_info")[0].reset();    
...
dateClick: function(info) {
    ...
    // Set the time of 'today' to midnight to only compare dates
    today.setHours(0, 0, 0, 0); 

    // Calculate the difference in time (in milliseconds) and convert to days
    var timeDiff = today - selectedDate;
    var diffDays = Math.floor(timeDiff / (1000 * 60 * 60 * 24));  // Convert milliseconds to days

    // Check if selected date is before today and within 7 days
    if (diffDays > 6) {
        $("#warndata").find('.modal-body').text('ไม่สามารถบันทึกข้อมูลย้อนหลังเกิน 7 วันได้');
        $("#warndata").modal('show');
        return;  
    }
...
function editwork(id,info_date,work) {	
	//alert(id);
	//alert(info_date);
	editdatework = info_date;

		$.ajax({
			 url: "get_work_info.php",
			 type: "POST",
			 data:  {
							 datework:info_date
							},
			 ...
			 success: function(data)
					{
						...
						if (data[0].chk == 1) {			
							...
							if(data[0].staff_approve==1) { 
								$("#frm_add_info input[id=staff_approve]").prop('checked',true);
								$("#frm_add_info textarea[name=work]").prop('readonly', true);
								$("#btn_del_info").hide();
								$("#btn_add_info").hide();
							}else if(data[0].staff_comment != "" && data[0].staff_comment != null) {
									$("#frm_add_info input[id=staff_approve]").prop('checked', false);
									$("#frm_add_info textarea[name=work]").prop('readonly', false);
									$("#btn_del_info").hide();
									$("#btn_add_info").show();
								}else{
						...
						if (data[0].chk == 1) {			
							$("#frm_add_info input[id=dateadd]").val(info_date);
							$("#frm_add_info textarea[name=work]").val(data[0].work);	
							if(data[0].auditor_approve==1) { 
								$("#frm_add_info input[id=auditor_approve]").prop('checked',true);
								$("#frm_add_info textarea[name=work]").prop('readonly', true);
								$("#btn_del_info").hide();
								$("#btn_add_info").hide();
							}else if(data[0].auditor_comment != "" && data[0].auditor_comment != null) {
									$("#frm_add_info input[id=auditor_approve]").prop('checked', false);
									$("#frm_add_info textarea[name=work]").prop('readonly', false);
									$("#btn_del_info").hide();
									$("#btn_add_info").show();
								}else{
...
<body>
    ...
    <nav class="navbar navbar-expand-lg navbar-dark ">
       <a class="navbar-brand" href= "main.php">ระบบฝึกประสบการณ์วิชาชีพ</a>
...
-->
			 ...
			 <li class="nav-item dropdown">
                ...
                ยินดีต้อนรับ, <?php echo $_SESSION['SESS_STD_NAME']; ?></a>              
				<div class="dropdown-menu" aria-labelledby="navbarDropdown">
					<a class="dropdown-item" href="edt_std_profile.php">ข้อมูลส่วนตัว</a>
					<a class="dropdown-item" href="edt_std_passwd.php">เปลี่ยนรหัสผ่าน</a>									
                </div>
             </li>
			  

			 <li class="nav-item">
                <a class="nav-link" href="main.php">บันทึกข้อมูลการฝึกประสบการณ์วิชาชีพ</a>
             </li>		  
			 <li class="nav-item">
...
-->
        <div class="col-md-12 pb-1 pb-md-0">
          <div class="card">
            <div class="card-body">
              <h5 class="card-title">บันทึกข้อมูลการฝึกประสบการณ์วิชาชีพ</h5>		
            </div>
          </div>
        </div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
		...
		<form id="frm_add_info" action="" method="POST">  
			...
			<div class="row">
			  ...
			  <div class="col-md-7">
				<textarea rows="8" id="work" name="work" class="form-control"></textarea>
			  </div>
			</div>
			...
			<div class="row">
			  ...
			  <div class="col-md-7">
				<input type="checkbox" name="staff_approve" id="staff_approve" value="1" disabled> รับทราบ</br>
			  </div>
			</div>
			...
			<div class="row">
			  ...
			  <div class="col-md-7">
				<textarea rows="8" id="staff_comment" name="staff_comment" class="form-control" readonly> </textarea>
			  </div>
			</div>			
			<div class="row">
      ...
      <div class="modal-footer">
		...
		<button type="button" class="btn btn-primary" id="btn_del_info" style="display: none;">ลบข้อมูล</button>
      </div>
    </div>
  </div>
</div>
...
<div class="modal fade" id="deldata" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">ยกเลิก</button>
		<button type="button" class="btn btn-primary" id="btn_condel_info">ลบข้อมูล</button>
      </div>
    </div>
  </div>
</div>

<div class="modal fade" id="warndata" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
